...
```json
{
  ...
  "expires_at": "2024-12-19T12:01:30Z"
}
```
...
**認証**: 不要  
**用途**: 30秒ごとに呼び出し、ロックを延長（1.5分延長）

**レスポンス（成功時）**:
```json
{
  "success": true,
  "expires_at": "2024-12-19T12:02:00Z"
}
```
...
### 4.3 編集中
```
1. ハートビートを30秒ごとに送信（PUT /api/documents/{roomId}/lock）
2. ハートビートが失敗した場合:
   - ロックが失われたと判断
   - エディタを読み取り専用にする
   - 「ロックが失われました」と表示
...
### 6.1 ロックの有効期限
- **ロック取得時**: 1.5分（90秒）の有効期限
- **ハートビート**: 30秒ごとに送信し、有効期限を1.5分に延長
- **タイムアウト**: ハートビートが停止すると、最後の延長から1.5分後に自動削除
